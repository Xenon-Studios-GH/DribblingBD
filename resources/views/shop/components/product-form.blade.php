    ...
    <div class="mt-6">
        ...
        <div class="flex flex-wrap gap-2 mt-2">
            @foreach ($productFormData['sizes'] as $size)
                ...
                @endphp
                <button @click='setSize(@json($size))' :class="selectedSize === '{{ $size }}' ? 'bg-[#E85D2C] text-white border-[#E85D2C]' : 'bg-white text-gray-700 border-gray-300 hover:border-[#E85D2C]'" class="px-4 py-2.5 rounded-xl border text-sm font-medium transition-colors">
                    {{ $size }}
                </button>
            @endforeach
        </div>
