...
<div x-data='productForm(@json($productFormData))'>
    {{-- Size selector --}}
    <div class="mt-6">
        ...
        <div class="flex flex-wrap gap-2 mt-2">
            @foreach ($productFormData['sizes'] as $size)
                ...
                @endphp
                <button @click='setSize(@json($size))' :class="selectedSize === @json($size) ? 'bg-[#E85D2C] text-white border-[#E85D2C]' : 'bg-white text-gray-700 border-gray-300 hover:border-[#E85D2C]'" class="px-4 py-2.5 rounded-xl border text-sm font-medium transition-colors">
                    {{ $size }}
                </button>
            @endforeach
        </div>
    ...
    <div class="mt-4">
        ...
        <div class="flex items-center gap-3 mt-2">
            <button @click="qty = Math.max(1, qty - 1)" :disabled="qty < 2" class="w-10 h-10 rounded-xl border border-gray-300 flex items-center justify-center text-gray-600 hover:bg-gray-50 transition-colors disabled:opacity-30 disabled:cursor-not-allowed">
                <i class="fas fa-minus w-4 h-4"></i>
            </button>
            <span class="w-12 text-center text-lg font-semibold text-gray-900" x-text="qty"></span>
            <button @click="qty = Math.min(100, qty + 1)" :disabled="qty >= 100" class="w-10 h-10 rounded-xl border border-gray-300 flex items-center justify-center text-gray-600 hover:bg-gray-50 transition-colors disabled:opacity-30 disabled:cursor-not-allowed">
