...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex items-center gap-2 text-sm text-gray-500 mb-6">
            ...
            <span>/</span>
            <span class="text-gray-900">{{ $category->parent ? $category->parent->name . ' > ' . $category->name : $category->name }}</span>
            @else
                    ...
                    <div class="group bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
                        <a href="{{ shop_project_url($project) }}" class="block">
                            <div class="aspect-square bg-gray-100 overflow-hidden">
                                ...
                                @if($firstImage)
                                <img src="{{ asset('storage/' . $firstImage->image_path) }}" alt="{{ $project->product->product_name }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300">
                                @else
