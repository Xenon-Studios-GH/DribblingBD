...
<body>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                <td><span class="{{ $t->type->value === 'in' ? 'badge-in' : 'badge-out' }}">{{ strtoupper($t->type->value) }}</span></td>
                <td>{{ $t->created_at->format('d M Y H:i') }}</td>
                <td>{{ $t->product?->product_name ?? 'Deleted' }}</td>
                <td>{{ $t->size }}</td>
                <td class="text-right {{ $t->type->value === 'in' ? 'text-green' : 'text-red' }}" style="font-weight:bold">{{ $t->type->value === 'in' ? '+' : '-' }}{{ number_format(abs($t->quantity)) }}</td>
                <td>{{ $t->note ?: '—' }}</td>
